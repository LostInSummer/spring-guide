...
import com.lostinsummer.spring.uploading.files.storage.StorageFileNotFoundException;
...
public class FileUploadController {
	...
	public String listUploadedFiles(Model model) throws IOException {
		...
		// Stream 是 Java 8 中对集合 Collection 的增强, 结合 lambda 用函数式编程对集合进行复杂的操作 (
		// 查找、遍历、过滤等)
		model.addAttribute("files",
				storageService.loadAll()
						...
						// Stream.collect 方法将 map转换后的新 Stream 变为 list 返回给模板
	...
	public ResponseEntity<Resource> serveFile(@PathVariable String filename) {
		...
		// CONTENT_DISPOSITION 该标志将通知浏览器启动下载
